...
func (r *objReader) init(b *bytes.Reader, p *Package) {
	r.b = b
	r.p = p
	r.offset, _ = b.Seek(0, io.SeekCurrent)
	r.limit = int64(b.Len())
	b.Seek(r.offset, io.SeekStart)
...
func (r *objReader) peek(n int) ([]byte, error) {
    ...
    r.b.Seek(r.offset, 0)
    buffReader := bufio.NewReader(r.b)
	b, err := buffReader.Peek(n)
	if err != nil {
...
func (r *objReader) skip(n int64) {
	...
	if n < int64(len(r.tmp)) {
		...
		r.readFull(r.tmp[:n])
	} else if n <= int64(buffReader.Buffered()) {
		...
		_, err := r.b.Seek(r.offset+n, io.SeekStart)
		if err != nil {
			r.error(err)
		}
		r.offset += n
...
func Parse(b *bytes.Reader, pkgpath string) (*Package, error) {
	if pkgpath == "" {
		pkgpath = `""`
	}
	p := new(Package)
...
func (r *objReader) parseObject(prefix []byte) error {
	...
	for i := int64(0); i < count; i++ {
        dwarfName := r.readString()
		r.p.DWARFFileList = append(r.p.DWARFFileList, dwarfName)
	}
	...
	for {
		...
		for i := range s.Reloc {
			...
			rel.Add = r.readInt()
			rel.Sym = r.readSymID()
            //log.Printf("rel.Sym %v", rel.Sym)
            r.patchDWARFName(&rel.Sym.Name)
		}
		...
		if s.Kind == objabi.STEXT {
			...
			for i := range f.FuncData {
                ...
                r.patchDWARFName(&f.FuncData[i].Sym.Name)
			}
			for i := range f.FuncData {
...
func (r *objReader) patchDWARFName(s *string) {
	// This is kind of ugly. Really the package name should not
	// even be included here.
    if strings.Contains(*s, "command-line-arguments."){
        *s = strings.ReplaceAll(*s, "command-line-arguments.", r.pkgprefix)
        //log.Printf("New Dwarf Name %s %s", *s, r.pkgprefix)
    }
